...
# ///
"""
Emit the wheel build matrix as JSON for the `release.yml` and `warm.yml`
plan jobs to publish.

Reads `platforms.toml`, keeps the entries that carry a `runner` (every
wheel row does, the sdist row does not), drops the fields no matrix
row consumes (`label`, `pattern`, `smoke`), and writes `matrix=<json>`
to `$GITHUB_OUTPUT` for the wheel-building jobs to read via

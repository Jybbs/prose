...
The same sweep holds each rule to the tree its input parses to. Every rewrite made by a rule declaring `PRESERVES_TREE`, pair runs included, has to parse to a `ComparableMod` equal to its input's, the `ruff_python_ast` comparison that ignores positions, parentheses, comments, and implicit string concatenation, and every such rule also runs together in one pipeline held to the same comparison. An equal tree means the rule moved only whitespace, punctuation, and comments, so for these rules the sweep covers every function body in the corpus, whereas a rule that changes the tree by design declares `false` and leaves `rust:imports` as its evidence. Each violation reports the rule, the file, and the width, with the diff at the innermost statement whose tree changed beside the command that sweeps that file alone. Over the fixture tree, the suite also fails on a rule declaring `false` whose every rewrite keeps the tree.

The `🪻 Corpus` workflow runs the sweeps against the pinned interpreter's standard library on every pull request touching the crate or the corpus binaries, the mutation pass as its own `🎨 Mutations` job beside `⚓ Settle`, restoring the stage `rust:bake` writes on each `main` push and the mutation set it writes once per revision of the generator, its manifest, or the lockfile, gating on the settle, mutation, import, and subset verdicts, rendering the delta into its job's log as a report, and running the subset probe as one row per rule family named by the family's badge, each scoped to the rules the pull request touches under `crate/src/rules/` where nothing else the workflow watches changed and carrying the whole family otherwise. The `🪟 Imports` job reads the break set tracked in the repository and fails only on a break that set does not carry or on a module that has fallen out of comparison since it was baked, so a module `main` already breaks does not fail a pull request that leaves it alone.

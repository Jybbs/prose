...
`rust:imports` checks what the settle probes cannot, whether a rewrite that settles still imports. It copies the corpus, formats the copy with `target-version` set to the interpreter's own version, and executes every importable module of the corpus from both trees in a fresh interpreter with its own tree first on `sys.path`, so a relative import resolves and a rewrite of one module reaches every module importing it, whether or not the formatter rewrote the importer. It runs as a `cargo test` target beside the settle and subset sweeps, formatting through the library rather than the binary, and the only Python it carries is the probe that loads one module inside the interpreter under test and reports the names it bound. A module counts as broken where the original imports cleanly and the formatted copy raises, times out, or binds a different namespace, with a second run of both sides confirming it. A file the pipeline read but could not format counts as broken too, whereas a file it could not read or parse is counted and left out, as the settle sweep leaves it. Where the second run of the original binds some names differently from the first (*a function address that address-space randomization moves, a timestamp taken on import*), those names are set aside rather than the whole module. The module still counts as broken wherever the formatted copy differs outside them, and a count of varying names never fails a run on its own.

The comparison leaves out a name the original bound and the formatted copy does not wherever a recorded fix removed its binding, whichever rule wrote that fix. Every module of the corpus runs, so one that reads the removed name raises as a break of its own, naming that module and the rule. A module outside the corpus that reads a removed name falls outside that evidence. Once a module runs, the probe reads its annotations through the attribute the way a consumer reads them, so the comparison covers which names a module annotates, and a formatted copy whose annotations raise when read counts as broken. The annotations of a class, which only a call such as `typing.get_type_hints` evaluates, fall outside what the probe reads.

The gate requires zero breaks, and it fails a run on any of the four conditions that leave it unable to judge one:

- A module left no record
- The format pass rewrote no file
- The run compared no module
- A module the original could not run raised from an import naming the module itself or a package holding it, which is the harness's loader failing rather than the module

The report keys each break by the frame it raises in, why it broke, and the rules whose fixes cover that line or dropped the binding of the name it misses, carrying one line per distinct defect with the modules it reached counted, the diff around the row it names, and the one-module command that reproduces it. It also lists each module the original could not run beside its exception, and each name the comparison left out beside the fix that removed it. `PROSE_IMPORTS_PYTHON` names the interpreter whose standard library the sweep runs, `PROSE_SETTLE_WIDTHS` adds widths beside the default, and `PROSE_IMPORTS_TIMEOUT` bounds one module's run.

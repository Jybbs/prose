...
/// stranded-padding walks each built on first read or carried across a
/// reparse from the source before it. `source_type` is the parse mode
/// and `line_ending` the sequence the text breaks its lines with,
/// leaving `cell_offsets` and `cell_numbers` to carry a notebook's cell

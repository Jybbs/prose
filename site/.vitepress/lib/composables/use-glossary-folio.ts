...
const filtered    = computed(() => folio.filterEntries(ordered, query.value))
const grouped     = computed(() => folio.groupByInitial(filtered.value))
...
function step(delta: number): void {
  ...
  const idx  = folio.cycleIndex(activeIndex.value, delta, pool.length)

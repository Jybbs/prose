...
export const glossary: Record<string, GlossaryEntry> = {
  ...
  'BindingAnalysis': {
    ...
    definition : '`BindingAnalysis` is a per-`Source` table indexing every write and read of '
               ...
               + 'reparse by a rule that keeps every binding. The `inlinable-bindings` rule '
               + 'consumes it.',
  ...
  'leading comment block': {
    ...
    definition : 'A leading comment block is a run of own-line `#` comments sitting directly '
               ...
               + 'above it and caps the run below at 1 blank line, kept where the block '
               + 'anchors in place or where the author left any and dropped to none '
               + 'otherwise. The orderer primitive\'s `block_range` carries the block '
               + 'with its item when reordering siblings.',
    families   : ['ordering'],
    rule       : 'space-statements'
  },

---
summary: 'Run-on-save against the editor’s save hook, or structured diagnostics surfaced inline in the LSP panel as you type.'
tagline: 'save · LSP'
---
...
The widget renders the per-editor snippet for the six editors *Prose* documents directly *(VSCode, Neovim, JetBrains, Sublime Text, Emacs, Helix)*. Three of those cards carry a dependency or a wrapper worth knowing before pasting the snippet:

- The VSCode card relies on the [**`emeraldwalk.RunOnSave`**](https://marketplace.visualstudio.com/items?itemName=emeraldwalk.RunOnSave) extension, which watches for save events and invokes the command on every match.
- The Neovim snippet wraps the shell-out in `silent!`, suppressing both the command prompt and any non-zero exit from blocking subsequent autocommands.
- The JetBrains snippet uses the *File Watchers* plugin, which invokes the binary on every save and replaces the buffer's content with the formatted output.

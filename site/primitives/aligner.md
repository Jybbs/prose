...
2. `Settings { line_length, max_shift, strip_singleton }` carries the rule's `[rules]` facets and its governing line cap. `From<&AlignmentConfig>` builds the canonical settings, `with_line_length` sets the cap a run resolves within, and `with_singleton_strip` flips the singleton-collapse behavior on.
3. `AlignWalker { groups: Vec<Vec<Edit>>, rule: RuleId, settings: Settings, source: &'a Source }` is the carrier each rule's visitor struct wraps. `AlignWalker::new(source, settings, rule)` builds one with an empty `groups` accumulator, where each entry is one fix the pipeline maps to a single diagnostic. `emit_if_candidate(members)` records a group's alignment edits when its rows form a candidate, the `group_edits(settings, members)` / `push_group` pair lets a rule fold extra edits into a group before committing it, and `is_held(anchor)` reports whether a row's line is skip-suppressed for `rule`.

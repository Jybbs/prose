...
2. `docstring_ranges(source) -> Vec<TextRange>` returns every docstring literal's range across the module, ascending by start. A rule walking a different surface reads it to tell docstring position from an ordinary literal, which is how [[normalize-literals]] keeps its quote facet off the frame `docstring-frame` owns.

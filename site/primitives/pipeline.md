...
*Pipeline* is the value `prose format` and `prose check` build and run. It carries the registered rules in their canonical order and reads them in a few ways. `run` splices the edits of each batch of consecutive rules the registry declares independent into a fresh buffer in one pass, reparses between batches so every later rule reads a settled AST, and returns the final [[source]] with a list of diagnostics and the set of rules that edited. `diagnose` collects every rule's findings against the source as written, for reporting. `settle_report` reads a buffer a run has already produced and names what still changes it, and `unsettled` returns the first part of that report alone.

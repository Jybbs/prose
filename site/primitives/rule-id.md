...
Each concrete rule lives under `prose::rules` (*a `pub(crate)` module today*). The registry macro in `crate/src/rules/registry.rs` emits a single source of truth that drives every consumer:

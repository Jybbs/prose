...
This rule lands last in the import-surface sequence, after [[bare-import-allowlist]] surfaces lint candidates on the bare-versus-from split, [[alphabetize]] sorts the entries, and [[blank-lines]] places the blank-line separators between groups. By the time this rule aligns the `import` keyword the block is already ordered and grouped, so the alignment lands cleanly without re-shuffling.

The same alignment primitive runs on three other separators in different contexts: [[align-equals]] on consecutive assignments, [[align-colons]] on dict literals and dataclass fields, [[match-case-align]] on single-expression case bodies.

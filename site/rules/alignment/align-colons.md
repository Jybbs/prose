...
The `:` separator sets a column of values beside a column of names wherever it appears, and `align-colons` gathers those contexts into a single shared alignment surface so all of them read as parallel two-column entries:

- Dictionary keys.
- Annotated assignments (*class fields alongside module- and function-scope variables*).

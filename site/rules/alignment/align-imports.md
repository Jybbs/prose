...
The rule reads each block as the run of consecutive imports at the same indentation. A blank line, a comment, or a non-import statement resets the run. [[alphabetize]] sorts the entries within each block and [[blank-lines]] separates the groups by category, both settling before this rule measures a column. [[bare-imports]] reports on the bare-versus-`from` choice without rewriting, so its finding is advice for the author rather than an input to this pass. The [**Pipeline Order**](/reference/pipeline-order) reference lists where each sits.

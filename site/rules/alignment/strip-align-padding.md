...
Beyond the pre-`:` gap, the rule settles the run after a colon to one space wherever that colon introduces a value, so a stray `x:   int` reads as `x: int` and a missing space in `x:int` fills to one. A `match`-arm body keeps the spacing [[align-match-case]] gives it, and a docstring `Args:` description stays as written.

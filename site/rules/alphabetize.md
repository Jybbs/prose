...
The ordering itself follows fixed per-construct conventions without per-project knobs. Method groups follow the dunders-properties-privates-publics rhythm. Pydantic fields follow required-then-optional. Imports sort alphabetically within each block. Set `docstring-entries = false` to skip the Google-style docstring-entry reorder while keeping every AST-level surface sorted.
...
<Fixture rule="alphabetize" case="docstring_args_entries" title="Google-Style `Args:` Entries Sort Alongside the Signature" />

...
The rule walks every `import` statement in the module, including nested ones inside function bodies, conditional blocks, and class bodies. An entry on the `allow` list preserves the bare form, including its dotted submodules (*`numpy.linalg` inherits the exemption from `numpy`*). When a downstream migration pass acts on the lint output, the rewrite hands off cleanly to the rest of the import surface: [**`alphabetize`**](/rules/alphabetize) sorts the resulting block, [**`align-imports`**](/rules/align-imports) aligns the `import` keyword, and [**`blank-lines`**](/rules/blank-lines) lands the gap between groups. The lint itself is non-rewriting, so the diagnostic surfaces without touching the source.
...
- [**`alphabetize`**](/rules/alphabetize) sorts the imports within each block.
- [**`align-imports`**](/rules/align-imports) aligns the `import` keyword across `from ... import ...` runs.
- [**`blank-lines`**](/rules/blank-lines) places the one-blank gap between adjacent bare-import and `from`-import groups.

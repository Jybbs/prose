...
Reflowed prose collapses every interior whitespace run to one space, leaving the word sequence identical on every run, whereas a section entry's aligned head stays verbatim, its description wrapping at the width [[strip-stranded-padding]] settles that head to where the strip drops the padding before a lone entry's `:`. Section prose wraps one line at a time, first-fit, so a row the pass writes re-wraps to itself, and a break that would seat a comment marker, an entry head, or a list marker at the next row's head folds back into the word before it, the bare `#`, `name:`, or `+` a wrap leaves at a row's end reading as the marker it is. A token reading as a URL or carrying an embedded `/` or `-` wraps as one atomic word, so an over-budget link overflows the measure intact rather than splitting between its segments.
...
An entry's head line and every line below it opening no entry of its own read as one paragraph, rewrapped from the description column the settled head leaves, so padding that moves the `:` reflows the whole description rather than stranding continuations. A line whose own shape marks it as a structure is exempt, so a doctest, a list item, or a bracketed literal under an entry keeps its layout.

...
The rule reads `docstring-line-length` for the description budget, `code-line-length` for the structured budget, and `docstring-structured-policy` to override the structured budget when a project prefers a single narrower line across the whole docstring. Code blocks inside the description (*fenced or indented*) are preserved verbatim, since their layout is load-bearing. The two sibling docstring rules sit upstream of this one: [**`no-single-line-docstrings`**](/rules/no-single-line-docstrings) expands single-line docstrings into the multi-line shape, then [**`multi-line-docstrings`**](/rules/multi-line-docstrings) lands the opener and closer on their own lines, and only then does this rule wrap the resulting body.
...
- [**`multi-line-docstrings`**](/rules/multi-line-docstrings) places the opening `"""` and closing `"""` on their own lines before wrapping runs.
- [**`no-single-line-docstrings`**](/rules/no-single-line-docstrings) expands single-line triple-quoted docstrings into the canonical multi-line shape this rule then wraps.

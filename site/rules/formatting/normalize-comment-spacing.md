...
`normalize-comment-spacing` settles a comment's opener onto a single space after the hash run and holds a trailing comment at least two columns clear of the code beside it, so the delimiter reads the same way on every line.

...
Three branches actually fire the rewrite. The file may carry zero annotations (*the directive is unused outright*). The `target-version` may be 3.14 or higher (*the runtime defers annotation evaluation, so the directive carries no runtime weight*). Or every name appearing in every annotation may resolve to an unconditional module-scope binding before its first annotation use (*forward references aren't needed, so the runtime evaluates annotations eagerly without raising*). When none of those branches holds, the import stays in place.

::: tabs key:prose-target-version
== Python 3.10
The version-gated branch stays quiet. Removal fires only if the file has zero annotations or every annotation resolves to an unconditional module-scope binding before use.

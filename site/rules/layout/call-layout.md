...
Where an exploded keyword's value is a `dict` or `list` an earlier pass already broke across lines, `call-layout` re-indents that block to the keyword column rather than splicing it in at its old position, its entries one step past the keyword and its closing bracket back at the keyword column. A value whose lines run through a string literal keeps its source shape, leaving the string's interior untouched.

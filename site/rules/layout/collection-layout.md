...
`collection-layout` expands a multi-entry collection to one entry per line once its entries cross the atomicity threshold, leaving a short collection of small entries on its row. It reaches dictionary, list, set, and tuple literals, a tuple only where it carries its parentheses.

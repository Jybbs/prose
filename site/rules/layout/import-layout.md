...
The rule acts on a single-line `from ... import ...` only. A `from ... import *`, a from-import already within budget, and a parenthesized multi-line import stay untouched, and a lone name whose own line still overflows keeps its place rather than splitting further. A backslash-continued import reaches this rule already rejoined, since [[shed-backslash-continuations]] sheds the escape ahead of it, so the split lands on the single line that rejoin produced. Pair with [[alphabetize]] to sort each roster before the split packs it, and with [[align-imports]] to align the `import` keyword across the resulting run, which already carries one identical prefix per line.

...
Every width the pass reads counts the separator closing an entry's row at the position [[alphabetize-siblings]] leaves it in, on the rejoin as well as the expansion, so an entry the sort moves last sheds the comma it carries and one the sort moves up gains one before either is measured, and the shape the pass picks holds once the sort lands. A literal written on one row measures at the width [[strip-stranded-padding]] settles it to, past the padding inside its brackets and at one space after each `:`, which is the width a rejoin writes it back at. A member the expansion moves keeps the calls inside it measured at the columns its rows land on, exploding one the move pushes past the budget in the same pass. A literal following one the pass expands on the same line measures where that expansion leaves it, on the closer's row at the statement's indent rather than under the continuation column the source wrote, and a dict value whose key the pass lays across rows measures from the key's last row.

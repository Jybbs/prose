...
A member the expansion keeps as written rather than laying out moves whole into the expanded form, its continuation rows shifted to the column the entries sit at, so a kept call or subscript reads under its siblings. A member whose rows align under its own opening bracket keeps the whole construct as written instead, and one running through a multi-line string keeps the string's own columns. A literal nested inside such a member, as an argument of a kept call or an operand beside it, still takes its own layout where its row lands, the one it would take as an entry of its own.

A dict expands once it has more than `max-dict-entries` entries whatever its width, and every collection enclosing it expands with it. A collection holding a call the count trigger of [[reflow-calls]] explodes expands the same way, since that call leaves the collection no one-row form to keep.

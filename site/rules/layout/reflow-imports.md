...
`reflow-imports` gives a long from-import a deliberate shape once it overflows the dedicated import budget, rewriting it into a run of `from ... import ...` statements. Each statement repeats the module prefix and greedily packs as many alphabetized names as fit before the next line opens, so the imported names begin at the column the eye reaches after `import` on every line and a deep module path never drives them rightward.

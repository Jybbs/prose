...
The decision is structural rather than textual, so a pair sheds only where removing it leaves both the parse and the grouping a reader sees unchanged. That test holds each of the following pairs in place:
...
3. The parentheses that form a one-element tuple, part of the tuple rather than a wrapper around it.
4. A pair holding one boolean operator inside a chain at the other, since `(a and b) or c` parses the same without it and then reads as three peers rather than two.
5. A pair whose interior carries a comment, since folding the break would strand the comment off the line it describes.

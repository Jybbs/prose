...
A line inside an import statement is measured against `import-line-length` and every other line against `code-line-length`, its trailing whitespace left out of the width. A row a layout rule will split is left to that rule, so `line-overflow` reports only what the layout rules leave over the cap. The constructs a layout rule splits are:

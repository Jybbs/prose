...
related : [strip-none-return, signature-layout, modernize-annotations]
layout  : doc
---
...
Because *Prose* reads source rather than resolving types, the rule never synthesizes an annotation for the author. When a confident local signal exists, a literal default *(`threshold=0.8` suggesting `float`)* or in-module call sites passing only literals, the report carries a suggestion the reader applies by hand. A bare `= None` default contributes its `| None` arm only alongside another signal, and conflicting or non-literal signals leave the report unsuggested. The suggestion is recorded as a **display-only** fix, recorded for the reader but never applied.

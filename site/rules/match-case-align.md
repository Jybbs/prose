...
The post-pattern `:` is one of four separator surfaces the alignment engine runs across. [**`align-colons`**](/rules/align-colons) covers the `:` in dict literals, dataclass fields, and function signatures. [**`align-equals`**](/rules/align-equals) covers the `=` sign on consecutive assignments. [**`align-imports`**](/rules/align-imports) covers the `import` keyword on `from ... import ...` blocks. When a `match` collapses to a single arm, [**`singleton-rule`**](/rules/singleton-rule) drops the padding so the result reads as plain code rather than a one-row dispatch table.

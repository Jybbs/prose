...
The rule fires on every multi-line docstring across module, class, and function scopes. Single-line docstrings (*opener, body, and closer all on one line*) are left alone for [**`no-single-line-docstrings`**](/rules/no-single-line-docstrings) to handle. Pair with [**`docstring-wrap`**](/rules/docstring-wrap) for the description-prose wrap that runs after this rule canonicalizes the opener and closer.
...
- [**`no-single-line-docstrings`**](/rules/no-single-line-docstrings) expands single-line docstrings into the multi-line shape this rule canonicalizes.
- [**`docstring-wrap`**](/rules/docstring-wrap) wraps description prose and structured sections to the configured budgets, after this rule places the opener and closer.

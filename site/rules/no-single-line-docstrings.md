...
The rule fires on module, class, and function single-line docstrings. The body content is preserved verbatim across the expansion, and the resulting multi-line form passes immediately to [**`multi-line-docstrings`**](/rules/multi-line-docstrings) for the opener-and-closer placement and to [**`docstring-wrap`**](/rules/docstring-wrap) for the line-budget wrap.
...
- [**`multi-line-docstrings`**](/rules/multi-line-docstrings) canonicalizes the opener and closer placement after this rule expands the body.
- [**`docstring-wrap`**](/rules/docstring-wrap) wraps the expanded description prose and structured sections to the configured budgets.

...
A section marker splits a run into sections that each sort on their own while the marker keeps its place, covering a banner (*`# --- Lifecycle ---`*), the same banner drawn with its rule closing the label rather than opening it (*`# Lifecycle -------#`*), a `##` heading, a suppression directive, a tool pragma (*`# isort: split`*), and a comment set at a shallower indent than the members around it. An ordinary comment is no divider and travels with the member below it, whereas a comment indented deeper than the member below it closes the member above and travels with that one, the way a note on the last line of a function body moves with the function. A group that packs several members onto one row keeps its order across a comment, because its members swap in place with every gap kept as written and the comment would stay put while they moved past it.

Positional-or-keyword parameters never reorder, since a slot is part of the call contract, whereas the keyword-only block past the `*` sorts, and [[unsorted-positionals]] reports a run out of order. A class whose header generates its constructor follows the same contract, so a `NamedTuple` or `msgspec.Struct` base and a `@dataclass`, `attrs`, or `pydantic.dataclasses` decorator each pin the field run, whereas `kw_only=True`, a `dataclasses.KW_ONLY` block, a `TypedDict`, and a `pydantic.BaseModel` sort throughout. A class whose field order carries meaning some other way, such as a `pandera.DataFrameModel` whose columns follow the declaration order, takes `# prose: keep` on its header. The marker holds the statements of the class body in the order written and orders the docstring entries naming a field to match the fields, whereas the body of each method, each unmarked nested class, and each dict, call, import name list, or dunder list inside a statement still sort.

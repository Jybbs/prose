...
related : [align-colons, align-imports, band-constants, bare-imports, blank-lines, group-imports, unsorted-positionals]
layout  : doc
---
...
A reader who already knows the codebase carries a **mental map** of where things live. When sibling members within a class, an enum, or a function call sit in arrival order, every reader builds a **different map**, which slows each new reader's first read. `alphabetize` gives everyone the **same landmarks**:
...
A class or function definition also holds its place behind any sibling it names at evaluation time (*a base class, a decorator, a parameter default, a non-deferred annotation, or a class-body value*), since sorting it ahead of that sibling would move the name out of scope and raise `NameError` on import. Any module-level statement that reads a definition binds the run the same way, so a bare call naming a class holds every definition around it in place. A **decorated** definition at module scope holds its source slot outright, because applying a decorator runs a call and sorting the definitions reorders those calls, whereas a decorated method inside a class body still sorts. A run whose references form a cycle stays in source order untouched.
...
Pair with [[align-imports]] to align the `import` keyword across the freshly-sorted block, with [[align-colons]] to align dataclass-field annotations, and with [[blank-lines]] for the blank-line discipline around class members and the single blank line between the import groups.

...
An own-line comment above a constant travels into the band with it, so a note stays attached to the value it describes. A decorative banner *(`# --- Configuration ---`)* is the exception, reading as a section divider that pins the constant beneath it, and a band never crosses such a marker into the section above it.

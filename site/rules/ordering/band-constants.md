...
An own-line comment above a member rides with it wherever the rule seats it, a constant, an import, and a definition alike, so a note stays attached to whatever it describes. A decorative banner *(`# --- Configuration ---`)* is the exception, reading as a section divider that pins the member beneath it, and a band never crosses such a marker into the section above it. Inside a notebook the cell boundary bounds the carry too, so a comment closing one cell stays where the author typed it while the member in the next cell bands without it.

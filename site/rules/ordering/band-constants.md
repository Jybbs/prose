...
An own-line comment above a constant rides into the band with it, so a note stays attached to the value it describes. A decorative banner *(`# --- Configuration ---`)* is the exception, reading as a section divider that pins the constant beneath it, and a band never crosses such a marker into the section above it. Inside a notebook the cell boundary bounds the carry too, so a comment closing one cell stays where the author typed it while the constant in the next cell bands without it.

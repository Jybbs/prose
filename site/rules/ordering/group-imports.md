...
A recognized **section marker** *(a hand-drawn banner like `# --- Typing ---` or a `##` hash heading)* divides a run into independent sections, so an author who grouped imports under a divider keeps that grouping and no import crosses the marker into the section above it. [[blank-lines]] owns the single blank line dividing one canonical section from the next, [[import-layout]] runs first so every module reaches the partition on its own line, and [[align-imports]] acts on the grouped result, aligning the `import` keyword within each section.

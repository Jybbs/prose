...
- [**`BindingAnalysis`**](/primitives/binding-analysis) is the per-`Source` table this rule reads, where every write and read of every name in every scope is indexed once and queried by consuming rules.
- [**`loose-constants`**](/rules/loose-constants) lints the module-level equivalent (`SCREAMING_CASE = literal` assignments that would read better as a structured shape).

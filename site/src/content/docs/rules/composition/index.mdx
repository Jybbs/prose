...
import CompositionCards from '../../../../components/rules/CompositionCards.astro'
import Fixture          from '../../../../components/fixtures/Fixture.astro'
...
One module-level constant carries the full composition story. The right-hand dict starts in a state that sets these rules in motion:
...
These rules fire in [**Pipeline Order**](/reference/pipeline-order) against the same block, reparsing between each so every rule downstream measures against the rewritten source rather than the original.
